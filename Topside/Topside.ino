...
// ── ESP-NOW: MAC address of the FLOAT ESP32 ─────────────────────────────────
...
void sendCommand(String cmd) {
  ...
  if (result == ESP_OK) {
    Serial.print("SEND_OK: ");   Serial.println(cmd);
  } else {
    Serial.print("SEND_FAIL: "); Serial.println(cmd);
  }
}
...
void OnRecv(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
  ...
  if (msg.startsWith("SENSOR:")) {
    ...
    Serial.print("SENSOR|"); Serial.print(pressure);   Serial.print("|");
    Serial.print(depth);     Serial.print("|");
    Serial.print(motorState);Serial.print("|");
    Serial.println(packetsReceived);
  } else if (msg.startsWith("ACK:")) {
    acksReceived++;
    Serial.print("ACK|"); Serial.print(msg.substring(4));
    Serial.print("|");    Serial.println(acksReceived);
  } else if (msg.startsWith("PROFILE_")) {
    Serial.print("PROFILE|"); Serial.println(msg);
  } else {
